...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['update_profile'])) {
        ...
        $avatar_path_to_db_root_relative = $current_user_data['avatar']; // Путь от корня сайта

        if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
            $upload_dir_from_root = 'uploads/avatars/'; // Путь от корня сайта
            $upload_dir_relative_to_script = '../' . $upload_dir_from_root; // Путь от текущего скрипта
                    ...
                    if (!in_array($file_extension, ['jpg', 'jpeg', 'png', 'gif'])) {
                        ...
                        if (move_uploaded_file($file_tmp_name, $destination_relative_to_script)) {
                            if (!empty($current_user_data['avatar']) && 
                                strpos($current_user_data['avatar'], 'default-avatar.jpg') === false && 
                                strpos($current_user_data['avatar'], 'icons8.png') === false && 
                                file_exists('../' . $current_user_data['avatar'])) { // Проверяем от текущего скрипта
                                unlink('../' . $current_user_data['avatar']);
                            }
        ...
        if (empty($errors)) {
            ...
            if ($update_stmt) {
                ...
                if ($update_stmt->execute()) {
                    ...
                    $_SESSION['user']['avatar'] = $avatar_path_to_db_root_relative; // Сохраняем путь от корня

                    // Обновляем $current_user_data для немедленного отображения
     ...
     header('Location: index.php'); // Перенаправляем на index.php в текущей папке (profile/)
     exit();
}
...
if (isset($_SESSION['user']['id'])) { // Проверка, что user существует в сессии
...
<head>
    ...
    <link rel="stylesheet" href="../style.css?v=<?php echo time(); ?>">  <!-- Путь к основному CSS из папки profile -->
    <link rel="stylesheet" href="profile_style_v2.css?v=<?php echo time(); ?>"> <!-- CSS профиля в текущей папке -->
            ...
            <div class="header">
                 ...
                 <div class="header-action">
                    ...
                    <nav class="profile">
                        <nav class="account">
                             <?php
                                $avatar_path_header = '../img/icons8.png'; // Дефолтный аватар, путь от profile/
                                if (isset($_SESSION['user']['avatar']) && !empty($_SESSION['user']['avatar'])) {
                                    // Путь в сессии хранится от корня (например, "uploads/avatars/file.jpg")
                                    // Для отображения из папки profile/ добавляем "../"
                            ...
                            <ul class="submenu">
                                <li class="user-info">
                                    <div class="user-avatar">
                                        <?php
                                            $avatar_submenu_path = '../img/default-avatar.jpg'; // Дефолтный
                                            if (isset($_SESSION['user']['avatar']) && !empty($_SESSION['user']['avatar'])) {
        ...
        <div class="profile-card-container">
            <div class="profile-card">
                <div class="profile-card-header">
                    <?php
                        $avatar_display_profile = '../img/default-avatar.jpg'; // Дефолтный
                        if (!empty($current_user_data['avatar'])) {
                            // Путь в БД хранится от корня
                            $path_check_profile_page = '../' . $current_user_data['avatar'];
                ...
                <div class="profile-card-content">
                    <div id="info" class="tab-content active">
                        ...
                        <form action="index.php" method="post" enctype="multipart/form-data" class="profile-form-v2">
                    ...
                    <div id="password" class="tab-content">
                        ...
                        <form action="index.php" method="post" class="profile-form-v2">
                             <div class="form-group-v2">
