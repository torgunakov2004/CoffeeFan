...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_product'])) {
        ...
        if (empty($errors_add)) {
            $sql_insert_product = "INSERT INTO `products` (title, price, image, composition, features, country_of_origin, weight_grams, roast_level, processing_method, is_popular) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
<body>
    <header class="admin-header">
       ...
       <div class="container">
        ...
        <div class="admin-content form-container" id="addProductFormContainer" style="display: none;">
            ...
            <form action="manage_products.php" method="post" enctype="multipart/form-data" class="admin-form" id="addProductForm">
                ...
                <div class="form-group">
                    <label for="country_of_origin_add">Страна происхождения:</label>
                    <select id="country_of_origin_add" name="country_of_origin">
                        <option value="">-- Не выбрано --</option>
                        <?php foreach ($all_countries as $country): ?>
                            <option value="<?php echo htmlspecialchars($country); ?>"><?php echo htmlspecialchars($country); ?></option>
                        <?php endforeach; ?>
                    </select>
                    <input type="text" name="country_of_origin_new" placeholder="Или введите новую страну" style="margin-top: 5px;">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="roast_level_add">Степень обжарки:</label>
                    <select id="roast_level_add" name="roast_level">
                        <option value="">-- Не выбрано --</option>
                        <?php foreach ($all_roast_levels as $level): ?>
                            <option value="<?php echo htmlspecialchars($level); ?>"><?php echo htmlspecialchars($level); ?></option>
                        <?php endforeach; ?>
                    </select>
                    <input type="text" name="roast_level_new" placeholder="Или введите новую степень" style="margin-top: 5px;">
                </div>
                <?php if ($processing_column_exists): ?>
                <div class="form-group">
                    <label for="processing_method_add">Метод обработки:</label>
                    <select id="processing_method_add" name="processing_method">
                        <option value="">-- Не выбрано --</option>
                    ...
                    <input type="text" name="processing_method_new" placeholder="Или введите новый метод" style="margin-top: 5px;">
                </div>
                <?php endif; ?>
                <div class="form-group">
            ...
            <table class="admin-table" id="productsTable">
                <thead>
                    <tr>
                        ...
                        <th class="sortable" data-sort="roast_level">Обжарка</th>
