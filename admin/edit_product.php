...
// --- Конец получения уникальных значений ---
...
if ($product_id_to_edit > 0) {
    ...
    if ($stmt_get) {
        ...
        // $product_data = $_POST; // Не нужно здесь, т.к. если ошибка загрузки, product_data останется null
    }
} else {
...
if (empty($errors) && $product_data === null) { // Дополнительная проверка, если $stmt_get не сработал
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_product'])) {
    ...
    if ($processing_column_exists) {
        ...
        // Если колонки нет, берем старое значение (которое должно быть NULL или пустым)
    ...
    if (empty($errors)) {
        $sql_update_product = "UPDATE `products` SET title = ?, price = ?, image = ?, composition = ?, features = ?, country_of_origin = ?, weight_grams = ?, roast_level = ?, processing_method = ?, is_popular = ? WHERE id = ?";
        ...
        if ($stmt_update) {
            ...
            $stmt_update->bind_param("sdssssissii", 
                ...
                $processing_method_to_db_edit, $is_popular_update, $id
            );
    ...
    if (!empty($errors)) {
        ...
        // но сохраняем ID и текущее изображение, если новое не загрузилось или была другая ошибка
        $form_data_on_error = $_POST;
        $form_data_on_error['id'] = $id;
        $form_data_on_error['image'] = $new_image_path_db; // Используем new_image_path_db, т.к. он либо старый, либо новый (если загрузка удалась, но др. ошибка)
        $product_data = array_merge($product_data, $form_data_on_error); // Сливаем, данные из POST имеют приоритет
    }
}
...
<body>
    ...
    <div class="container">
        <div class="admin-content">
            ...
            <?php endif; ?>
            <?php if (empty($message) && !empty($errors)): // Если $message уже установлен, не дублируем ошибки ?>
                <div class="message error"><?php echo implode("<br>", array_map('htmlspecialchars', $errors)); ?></div>
            <?php endif; ?>


            <?php if ($product_data): // Отображаем форму только если данные продукта загружены ?>
