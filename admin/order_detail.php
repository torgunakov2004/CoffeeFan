    ...
    if ($order_id_to_update > 0 && in_array($new_status, $allowed_statuses)) {
        ...
        if ($stmt_update_status) {
            ...
            if ($stmt_update_status->execute()) {
                ...
                $_SESSION['admin_message'] = "Ошибка обновления статуса заказа: " . $stmt_update_status->error;
                $_SESSION['admin_message_type'] = "error";
            }
...
// Получаем основную информацию о заказе и клиенте
...
if ($stmt_items) {
    ...
    // Ошибка загрузки товаров, но страницу заказа все равно покажем
    error_log("Order Detail: Failed to prepare order items query: " . $connect->error);
}

function translateOrderStatusDetailAdmin($status) {
    // Можно использовать ту же функцию, что и в manage_orders.php, если она доступна
    // или определить ее здесь заново.
        ...
        <?php if ($order_details): ?>
            <div class="order-detail-card">
                <div class="order-detail-header">
                    ...
                    <p class="order-meta">
                        Дата: <strong><?php echo htmlspecialchars(date('d.m.Y H:i', strtotime($order_details['created_at']))) . ' (UTC)'; ?></strong> |
                ...
                <div class="info-block">
                    <h4>Изменить статус заказа</h4>
                     <form action="order_detail.php?id=<?php echo htmlspecialchars($order_details['id']); ?>" method="post" class="status-form-detail">
                      <input type="hidden" name="order_id" value="<?php echo htmlspecialchars($order_details['id']); ?>">
                        ...
                        <button type="submit" name="update_order_status" class="btn-save">Обновить статус</button>
                    </form>
                </div>
