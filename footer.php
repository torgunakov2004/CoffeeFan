...
document.addEventListener('DOMContentLoaded', function() {
    ...
    <script>
                // В footer.php или общем JS-файле
        $(document).ready(function() {
            // ... ваш другой JS-код ...

            // Определение и отправка часового пояса пользователя
            // Проверяем, не отправляли ли мы уже таймзону в этой сессии (чтобы не делать это при каждой загрузке)
            // Для этого можно использовать sessionStorage браузера или проверять наличие $_SESSION['user_timezone_js'] на сервере
            // Простой вариант: отправляем один раз, если в сессии PHP еще нет user_timezone_js
            ...
            try {
                ...
                if (userTimeZoneFromJS) {
                    // Отправляем на сервер, только если его еще нет в PHP сессии
                    console.log('Detected client timezone:', userTimeZoneFromJS); // Для отладки
                    $.ajax({
                        type: 'POST',
                        url: '<?php echo $base_web_path; // Используйте вашу переменную для базового URL ?>/set_user_timezone.php', // Путь к скрипту от корня сайта
                        data: { timezone: userTimeZoneFromJS },
                        dataType: 'json',
                        success: function(response) {
                            if (response && response.status === 'success') {
                                console.log('User timezone set on server:', response.timezone_set);
                                // Можно перезагрузить страницу, чтобы PHP сразу использовал новую таймзону из сессии
                                // window.location.reload();
                                // Но это может быть навязчиво. Лучше, если PHP будет использовать ее при следующем запросе.
                                // Для текущей загруженной страницы PHP уже использовал старое значение (или дефолт).
